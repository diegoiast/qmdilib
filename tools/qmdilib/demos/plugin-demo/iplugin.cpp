 ...
 * nothing more then a more specialized qmdiClient, with much more extra methods.
 * 
 ...
 * kind of "url": \b help:QString , \b help:///usr/share/doc/qt4/doc/html/opensourceedition\b .html
 * while it cannot directly open "help files" from the open file dialog.
 * 
 ...
 * 
 * The plugin manager can also disable or enable a plugin by calling setEnabled()
 * unless the plugin is marked as IPlugin::alwaysEnabled
 * 
 ...
 * it to the plugin.
 * 
 * The "new actions" will be displayed in the "File" menu of the plugin manager's
 * main window, as a sub menu, File->New-> (your actions). In this sub menu the
 * user will see all the "new actions" of all the available plugins. The order 
 * of appearence in that sub menu is not deterministic - the first which gets 
 * loaded is first to be displayed (this may change in future releases).
 * 
 * Example
 *\code
 * newplugin::newplugin()
 ...
 * {
 * 	QStringList s;
 * 	s << tr("Sources"	, "newplugin::myExtensions")	+ " (*.c *.cpp *.cxx *.h *.hpp *.hxx *.inc)";
 * 	s << tr("Headers"	, "newplugin::myExtensions")	+ " (*.h *.hpp *.hxx *.inc)";
 * 	s << tr("Qt project"	, "newplugin::myExtensions")	+ " (*.pro *.pri)";
 * 	s << tr("All files"	, "newplugin::myExtensions")	+ " (*.*)";
 * 	return s;
 * }
 * \endcode
 * 
 ...
 * \see IPlugin::enabled
 */
void	IPlugin::setEnabled( bool enable )
{
	enabled = enable;
}
